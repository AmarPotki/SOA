...
namespace RahyabServices.DataAccess.Repositories.TatCharity.Implementations{
    public class TatPensionListRepository : SharepointRepositoryBase<TatPension>, ITatPensionListRepository
    {
        private readonly IDataContextFactory _dataContextFactory;
        public TatPensionListRepository(IDataContextFactory databaseFactory) : base(databaseFactory){          
            Url = "http://";
            _dataContextFactory = databaseFactory;
            SharepointCredential = new NetworkCredential("", "", "");
        }
        
        public IEnumerable<TatPension> GetApplicantPensions(string id){
            ServicePointManager
...
(sender, cert, chain, sslPolicyErrors) => true;
         ...
         <Where> <Eq>
            ...
            client.Load(items);
            client.ExecuteQuery();
            return items.ProjectToCollectionEntity<TatPension>();
        }
    }
}